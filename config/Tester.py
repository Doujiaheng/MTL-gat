...
class Tester(object):
    ...
    def __init__(self, model = None,  data_loader = None, use_gpu = True, dflag = -1):
        if dflag == -1:
            base_add = 'Base.so'
        else:
            base_add = 'Base1.so'
        base_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "../utils/release/", base_add))
